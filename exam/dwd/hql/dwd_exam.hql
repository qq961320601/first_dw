...
select t3.id as exam_id,
       ...
       t1.examinee_name,
       t4.question_id,
       null as question_stage,
       t6.name as question_difficulty_name,
       t7.is_objective,
       t8.per_question_mark,
       nvl(if(t9.score is null,t10.score,t9.score),0) as score,
       if(nvl(if(t9.score is null,t10.score,t9.score),0) is not null and nvl(if(t9.score is null,t10.score,t9.score),0) <>'','正确','不正确') as is_current
 ...
 inner join (select  o1.examinee_id,
                     o1.examinee_name,
                     d3.question_id,
                     get_json_object(o1.subject_answer_json,CONCAT('$.',cast(d3.question_id as string),'.score'))  as score
               from  ods_exam.ods_answer_paper o1
               inner join ods_exam.ods_paper_question d3
